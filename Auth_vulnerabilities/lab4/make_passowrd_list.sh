...
for i in "${!passwords[@]}"; do
    if (( i % 2 == 0 )); then
        echo "wiener" >> usernames_list.txt
        echo "peter" >> passwords_list.txt
    fi
    echo "carlos" >> usernames_list.txt
    echo "${passwords[$i]}" >> passwords_list.txt
done

echo "Généré $(wc -l < usernames_list.txt) lignes"